...
public sealed class FileMoveCommandHandler : ICommandHandler
{
    ...
    public ICommand? Handle(ITokenIterator iterator, IPathParser pathParser)
    {
        ...
        if (!string.Equals(iterator.Current, "move", StringComparison.OrdinalIgnoreCase))
            return _next?.Handle(iterator, pathParser);

        if (!iterator.MoveNext())
            return new ErrorCommand("file move requires [SourcePath] [DestinationPath]");
    ...
    private sealed class FileMoveCommand : ICommand
    {
        private readonly IPath _source;
        private readonly IPath _destinationDirectory;

        public FileMoveCommand(IPath source, IPath destinationDirectory)
        {
        ...
        public CommandResult Execute(SessionContext context)
        {
            ...
            return context.FileSystem.MoveFile(srcOk.Path, dstOk.Path);
        }
    }
}
